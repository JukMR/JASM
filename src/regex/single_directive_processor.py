...
class SingleDirectiveProcessor(DirectiveProcessor):
    ...
    def __init__(self, basic_pattern: PatternDict) -> None:
        include_list = self._get_mnemonic_from_simple_pattern(
    ...
    @staticmethod
    def _get_instruction_properties(include_list: IncludeExcludeListType,
                                    pattern: PatternDict) -> Dict[str, Any]:
        if include_list is None:
            raise ValueError(f"Some error happened and this basic instruction: {pattern}"
        ...
        if not isinstance(instruction_properties, Dict):
            raise ValueError(
                f"Returning properties {instruction_properties} is not a Dict")
