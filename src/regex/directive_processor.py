...
class DirectiveProcessor:
    ...
    def _generate_only_include(include_list_regex: List[str], times_regex: Optional[str], operands: str) -> str:
        inst_joined = join_instructions(inst_list=include_list_regex, operand=operands)
    ...
    @staticmethod
    def _generate_only_exclude(exclude_list_regex: List[str], times_regex: Optional[str], operands: str) -> str:
        inst_joined = join_instructions(inst_list=exclude_list_regex, operand=operands)

        if times_regex is None:
            return f"((?!{inst_joined}){operands})"

        return f"((?!{inst_joined}){operands}){times_regex}"
    ...
    def _get_min_max_regex(self) -> Optional[str]:
        ...
        if min_amount > max_amount:
            raise ValueError(f"Wrong min:{min_amount} or max:{max_amount} in directive")
    ...
    def _process_any(
        ...
        exclude_list_regex = join_instructions(inst_list=exclude_list, operand=operands)
        include_list_regex = join_instructions(inst_list=include_list, operand=operands)
    ...
    def _process_not(self, times_regex: Optional[str], exclude_list: List[str], operands: str) -> str:
        return self._generate_only_exclude(exclude_list_regex=exclude_list, times_regex=times_regex, operands=operands)

    def _process_simple(self, times_regex: Optional[str], include_list: List[str], operands: str) -> str:
        return self._generate_only_include(include_list_regex=include_list, times_regex=times_regex, operands=operands)
    ...
    def process(self) -> str:
        ...
        if self.exclude_list is not None and self.include_list is not None:
            return self._process_any(
                times_regex=times_regex_str,
                operands=operand_regex,
                include_list=self.include_list,
                exclude_list=self.exclude_list,
            )
        if self.exclude_list is not None:
            return self._process_not(
                times_regex=times_regex_str,
                operands=operand_regex,
                exclude_list=self.exclude_list,
            )
