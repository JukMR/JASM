...
class CaptureGroupIndex(ABC):
    ...
    def __init__(self, pattern_node: PatternNode, str_index: str) -> None:
        ...
        assert pattern_node.shared_context.capture_manager
        assert isinstance(pattern_node.shared_context.capture_manager, CapturesManager)

        assert (
            hasattr(pattern_node.shared_context.capture_manager, "capture_group_references")
            and pattern_node.shared_context.capture_manager._capture_group_references is not None
        )
