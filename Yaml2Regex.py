...
class InstructionProcessor:
    ...
    def process_list(self, list_type: str, any_com: Command) -> Tuple[str, int, int]:
        if 'include_list' in any_com:
            list_type = 'include_list'
        elif 'exclude_list' in any_com:
            list_type = 'exclude_list'
        else:
            raise ValueError(f"{list_type} not in {any_com}")

        output = ''
        for elem in any_com[list_type]:
        ...
        return output, min_amount, max_amount




    def process_any(self, any_com: Command) -> str:
        self.process_list(an)
