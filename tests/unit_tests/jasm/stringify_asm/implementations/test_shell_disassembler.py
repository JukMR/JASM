...
def test_disassemble_success(mock_run, mock_exists, shell_disassembler: ShellDisassembler) -> None:
...
def test_disassemble_success_log(mock_info, mock_run, mock_exists, shell_disassembler):
    mock_run.return_value = MagicMock(returncode=0, stdout="disassembled content")
    input_file = "path/to/binary"
    result = shell_disassembler.disassemble(input_file)
    assert result == "disassembled content"
...
def test_disassemble_file_not_found(mock_exists, shell_disassembler: ShellDisassembler) -> None:
...
def test_disassemble_program_not_found(mock_error, mock_run, mock_exists, shell_disassembler):
    input_file = "path/to/binary"
...
@patch("jasm.logging_config.logger.error")
def test_disassemble_general_error(mock_error, mock_run, mock_exists, shell_disassembler):
    input_file = "path/to/binary"
