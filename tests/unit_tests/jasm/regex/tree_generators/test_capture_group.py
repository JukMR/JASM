...
from jasm.jasm_regex.tree_generators.capture_group_index import (
    CaptureGroupIndexInstructionCall,
    CaptureGroupIndexOperandCall,
    CaptureGroupIndexRegisterCall
)
...
@pytest.fixture
def mock_pattern_node(mock_shared_context: SharedContext) -> Mock:
    node = Mock()
    node.name = "1"
...
def test_initialization_with_valid_node(mock_pattern_node: Mock) -> None:
    cgi = CaptureGroupIndexInstructionCall(mock_pattern_node)
    assert cgi.index == 1
    assert isinstance(cgi, CaptureGroupIndexInstructionCall)


def test_initialization_without_capture_group_references(mock_pattern_node: Mock) -> None:
    shared_context = SharedContext(CapturesManager())
...
def test_to_regex_instruction(mock_pattern_node: Mock) -> None:
    cgi = CaptureGroupIndexInstructionCall(mock_pattern_node)
    expected_regex = rf"{IGNORE_INST_ADDR}\{cgi.index},\|"
    assert cgi.to_regex() == expected_regex


def test_to_regex_operand(mock_pattern_node: Mock) -> None:
    cgi = CaptureGroupIndexOperandCall(mock_pattern_node)
    expected_regex = rf"\{cgi.index},?"
    assert cgi.to_regex() == expected_regex


def test_to_regex_register(mock_pattern_node: Mock) -> None:
    cgi = CaptureGroupIndexRegisterCall(mock_pattern_node)
    expected_regex = rf"\{cgi.index},?"
    assert cgi.to_regex() == expected_regex
