...
def test_pattern_node_builder_no_parents_with_dict(mock_shared_context):
    ...
    assert builder.times == TimesType(min_times=1, max_times=3)
    ...
    assert builder.children[0].times == TimesType(min_times=1, max_times=1)
    assert builder.children[1].times == TimesType(min_times=1, max_times=1)
...
@pytest.mark.parametrize(
    ...
    [
        ({"min": 2, "max": 4}, TimesType(min_times=2, max_times=4)),
        ({"min": 3, "max": 3}, TimesType(min_times=3, max_times=3)),
        ({}, TimesType(min_times=1, max_times=1)),  # Testing default behavior
    ],
)
